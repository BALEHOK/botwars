...
function step(stepNum, bots, field, stateSubject) {
  ...
  if (move[0] < 0 || move[0] >= fieldSize
    ...
    const result = new GameResult(
      signs[Number(!botIndex)] + ' win',
      botIndex && [0, 1] || [1, 0],
      botIndex
    );
    stateSubject.next(new GameState(GameStateEnum.finished, [...field], result, sign + ' returned error move'));
    return;
  }
  ...
  if (hasWon(sign, move, field)) {
    ...
    stateSubject.next(new GameState(GameStateEnum.finished, [...field], result));
    return;
  }
